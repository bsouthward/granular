...
var release = 0.40;
var density = 0.85;
var spread = 0.2; // Spread between grain start and end
var reverb = 0.5;
var pan = 0.1;
var trans = 1;

var queryInterval = 2000; // Frequency we query the current sound file (in millisecnds)


const owAPIKey = "033c9c2e06f99aef2fe57093880b686f";

var startTime = 1459753200;
var endTime = 1459839600;

var grainDuration = spread;
var grainSpacing = 0.5 * spread;

var latitudeTulsa = 36.15;
var longitudeTulsa = 95.99;

var latitudeSF = 37.77;
var longitudeSF = 122.41;

var speed = 0.1;

// This function queries openweathermap for data
function weatherFromCoordinates(latitude, longitude, callback) {
    var reqURL = `http://api.openweathermap.org/data/2.5/weather?q=lat=${latitude}&$lon=${longitude}&appid=033c9c2e06f99aef2fe57093880b686f`;
...
// This is called when the API returns it's update
function updateParameters(result) {
    ...
    spread = convertHumidityToSpread(humidity)
    speed  = convertKelvinToGrainSeconds(temperature);
    queryInterval = converthPaToGrainRandomSpeed(pressure);

    console.log("spread", spread)
    console.log("queryInterva", queryInterval)
}

function convertHumidityToSpread(value) {
    return value / 100;
}

function convertKelvinToGrainSeconds(value) {
    return ((value - 200) / (350 - 200)) * (3 - 0.01) + 0.01;
}

function converthPaToGrainRandomSpeed(value) {
    return ((value - 800) / (1100 - 800)) * (3000 - 700) + 700;
}
...
weatherFromCoordinates(latitudeTulsa, longitudeTulsa, updateParameters);
...
function playNext(p) {
    var positionX = currentPosition + Math.pow((1+spread), 3);
    this.grains = [];
    this.grainscount = 0;
    var that = this; //for scope issues
    this.play = function(){
        //create new grain
        var g = new grain(p,buffer,positionX,50,attack,release,spread,pan);
        //push to the array
...
voice.prototype.playmouse = function(p){
    setInterval(() => playNext(p), queryInterval)
    return;
    this.grains = [];
    this.grainscount = 0;
    var that = this; //for scope issues
